...
* channel: ${channel?.title}`);
  ...
  getStream({ seek, filter }: StreamOptions) {
    if (filter)
      return {
...
* album: ${album.name} (${albumURL})`
  ...
  static async fromListURL(
    ...
    return Promise.all(
      tracks.map(track => SpotifySong.fromId(track.id, requester))
    );
  }
...
* user: ${user.name} (${user.url})`);
  ...
  static fromId(id: string, requester: Requester): Promise<SoundCloudSong> {
    const url = SpotifySong.id2URL(id);
    return this.fromURL(url, requester);
  }
